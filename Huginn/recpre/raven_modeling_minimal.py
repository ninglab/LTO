...
from .score import CoEScoreInfo
import secrets

global_tokenizer = AutoTokenizer.from_pretrained("tomg-group-umd/huginn-0125")
...
class RavenForCausalLM(RavenPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs,
        ...
        print(global_tokenizer.batch_decode(input_ids))
    ...
    def initialize_state(self, input_embeds, deterministic: bool = False):
        ...
        torch.manual_seed(secrets.randbits(10))
        torch.cuda.manual_seed(secrets.randbits(10)) 
        torch.nn.init.trunc_normal_(x, mean=0.0, std=std+secrets.randbits(9)/2**12, a=-3 * std, b=3 * std)
        if self.emb_scale != 1:
            x = x * self.emb_scale
    ...
    def generate_with_adaptive_compute(
        ...
        **model_kwargs,
        ...
        #print(model_kwargs["num_steps"])
        all_pos_hs = [[] for compute_step in range(model_kwargs["num_steps"] + 1)]
        '''
        ...
        '''
        # Generate tokens
        input_ids_original_length = input_ids.shape[-1]
        for step in range(min(generation_config.max_length - input_ids.shape[1], generation_config.max_new_tokens)):
            ...
            #print("current_latents.shape:", current_latents.shape)    
            #print("embedding_input.shape:", embedded_inputs.shape)
            ...
            for compute_step in range(model_inputs["num_steps"]):
                #print("we are in the normal loop!")
                #print("current_latents.shape:", current_latents.shape) 
                prev_latents = current_latents.clone()
                #print(layer_idx)
                all_pos_hs[layer_idx].append(self.transformer.ln_f(prev_latents)[0][-1].cpu().float())
                layer_idx += 1
                ...
                if step > 100:  # do not exit in prefill:
                    ...
                    new_exits = torch.zeros_like(new_exits, dtype=torch.bool)
            ...
            # This else is if the for loop finished without breaking
                ...
                #print("we are in the for else loop!")
                #print(layer_idx)
                #print("current_latents.shape:", current_latents.shape) 
                all_pos_hs[layer_idx].append(self.transformer.ln_f(current_latents)[0][-1].cpu().float())
                layer_idx += 1
            ...
            #print(next_token)
            #print(global_tokenizer.batch_decode(input_ids))
            next_tokens = global_tokenizer.batch_decode(next_token)
            ...
            for i in range(batch_size):
                ...
                if next_tokens[i] in ['Question', 'question' , 'Question:', 'Q', '</s>', '<|im_end|>', '<|end_text|>', '<|end_turn|>']:
                    finished_sequences[i] = True
                    print("finished")
        ...
        if streamer:
            streamer.end()
        #temp = np.array(all_pos_hs)
        #print("all_pos_hs.shape", temp.shape)
        #temp = torch.stack([torch.stack(all_pos_hs[i], dim=0) for i in range(len(all_pos_hs))], dim=0)
        temp = np.array(all_pos_hs)[:,:-1,:]
        latent_embeddings = np.mean(temp, axis=1)
        #latent_embeddings = torch.mean(temp, dim=1).cpu().numpy()
        
        #coescoreinfo = CoEScoreInfo(latent_embeddings)
        #al_repdiff_norm, al_repdiff_ave, al_repdiff_var = coescoreinfo.compute_CoE_Mag()
        #al_semdiff_norm, al_semdiff_ave, al_semdiff_var = coescoreinfo.compute_CoE_Ang()
        #print("al_repdiff_norm, al_repdiff_ave, al_repdiff_var", al_repdiff_norm, al_repdiff_ave, al_repdiff_var)
        #print("al_semdiff_norm, al_semdiff_ave, al_semdiff_var", al_semdiff_norm, al_semdiff_ave, al_semdiff_var)
        #coe_r = coescoreinfo.compute_CoE_R()
        #coe_c = coescoreinfo.compute_CoE_C()
        #print("coe_r", coe_r)
        #print("coe_c", coe_c)
        my_args["al_repdiff_norm"] = [0.0] * temp.shape[0]
        my_args["al_repdiff_ave"] = 0.0
        my_args["al_repdiff_var"] = 0.0
        my_args["al_semdiff_norm"] = [0.0] * temp.shape[0]
        my_args["al_semdiff_ave"] = 0.0
        my_args["al_semdiff_var"] = 0.0
        my_args["coe_r"] = 0.0
        my_args["coe_c"] = 0.0
        
        
        
        #layer_num = temp.shape[0]
        #norm_denominator = compute(temp[-1]) - compute(temp[0])
        #al_repdiff_info = np.array([compute(temp[i+1])- compute(temp[i+1]) for i in range(layer_num - 1)])
        #print("norm_denominator", norm_denominator)
        #print("al_repdiff_info", al_repdiff_info)
        
        output_ids = input_ids[..., input_ids_original_length:-1]
        my_args["latent_embeddings"] = temp
        my_args["output_ids"] = output_ids.cpu().numpy()
        #print(global_tokenizer.batch_decode(input_ids))
        if generation_config.return_dict_in_generate:
            return GenerateDecoderOnlyOutput(
                sequences=input_ids,
        ...
        #return input_ids
        #print("id.shape:", input_ids[..., input_ids_original_length:-1].shape)
        #print("latent.shape:", np.array(all_pos_hs)[:,:-1,:].shape)
        return output_ids
